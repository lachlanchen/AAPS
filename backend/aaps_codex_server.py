    ...
    pack = {
        "contract": {
            ...
            "strict_rules": [
                ...
                "Default block archetypes are intent/context, input discovery, QC guard, method router, loop/batch, code/script action, agent action, agent handoff chain, image mask refinement, validation/recovery, and report/artifact.",
                "Chat refinements should patch the existing manifested workflow, scripts, prompts, registries, and report builders; create brand-new implementation files only for missing components or explicit new-artifact requests.",
                "Image mask refinement prompts must state reference-image policy and forbid embedded text, labels, arrows, numbers, captions, legends, and whole-cluster single-object annotation unless annotation is explicitly requested.",
...
- Image mask refinement prompts must identify the reference-image policy. By default, use the original/source image as the visual reference, describe prior masks/overlays as text-only QC context, and require colored instance-mask output with no embedded text, labels, arrows, numbers, captions, legends, table/grid labels, or whole-cluster single-object annotation.
...
- For image-mask handoffs, define which image is the visual reference. Default to the original/source image as the only visual reference; describe Cellpose/threshold masks or overlays as text-only QC context unless a verifier requests extra visual references. For colored instance masks, forbid embedded text, labels, arrows, numbers, captions, legends, table/grid labels, and whole-cluster single-object annotation.
