...
- Image mask refinement prompts must identify the reference-image policy. By default, use the original/source image as the visual reference, describe prior masks/overlays as text-only QC context, and require colored instance-mask output on a plain black/white/transparent background with no grayscale microscopy underlay, no embedded text, labels, arrows, numbers, captions, legends, table/grid labels, or whole-cluster single-object annotation.
