...
Default archetypes include intent/context, input discovery, QC guard, method/tool/agent router, loop/batch, code or script action, agent action, agent handoff chain, image mask refinement, validation/recovery, and report recap/artifact blocks. Custom blocks are encouraged, but they should still declare typed inputs, outputs, executable actions or agent prompts, environment/tool requirements, validations, recovery, artifacts, and review checkpoints. Chat-driven refinements should patch the existing manifested `.aaps`, scripts, prompts, registries, and report builders; manifest creates brand-new files only for missing components or explicit new-artifact requests. Agent handoff chains should carry source images/artifacts, upstream conclusions, QC findings, failure reasons, high-quality prompts, expected schemas, integration manifests, and verification criteria across Codex, AgInTiFlow, image-generation agents, and verifier agents. Image mask refinement blocks should declare reference-image policy, use a plain mask background unless explicitly configured as an overlay, and forbid embedded text, labels, arrows, captions, legends, and whole-cluster single-object annotation unless the task explicitly asks for annotation. Parser diagnostics are a hard agent-completion gate: failed `.aaps` diagnostics must be fed back into the repair loop until the shared parser is clean. Report recap blocks should consume input evidence, method comparisons, QC decisions, handoff packets, logs, validation summaries, checkpoints, and final artifacts. See [docs/block-design.md](docs/block-design.md), [docs/agent-handoff.md](docs/agent-handoff.md), and [docs/report-paradigm.md](docs/report-paradigm.md).
