(function (root, factory) {
  ...
  const AGENT_HANDOFF_PRINCIPLES = [
    ...
    "Mask-generation prompts should identify which images are references versus text-only QC context. By default, use the original source image as the visual reference and describe previous masks/overlays in text unless the verifier explicitly asks for extra visual references.",
    "For colored instance mask outputs, require one flat distinct color per visible instance, source-aligned dimensions, a plain black/white/transparent background, no microscopy texture underlay unless the block explicitly asks for an overlay, and no text, labels, arrows, captions, legends, tables, or whole-cluster single-object annotation inside the image.",
  ...
  const AGENT_HANDOFF_PACKET_SCHEMA = {
    ...
    referencePolicy: "which images are visual references, which artifacts are text-only context, and whether extra references are allowed",
    upstreamConclusion: {
    ...
    visualOutputContract: {
      noEmbeddedText: true,
      maskOnly: true,
      backgroundPolicy: "plain black, white, or transparent background for mask artifacts",
      noMicroscopyUnderlay: true,
      instanceColoring: "one distinct flat color per instance when the task is an instance mask",
      dimensionPolicy: "match source image dimensions unless explicitly declared otherwise",
    },
  ...
  const REPORT_RECAP_PRINCIPLES = [
    ...
    "Report figures should not bake explanatory labels into generated image grids unless the figure itself is an annotation task. Prefer clean image panels and put titles, captions, legends, and method names in the surrounding TeX/Markdown.",
  ];
  const REPORT_RECAP_PROMPT = [
    ...
    "For generated or refined image masks, state the reference policy and confirm that report grids are clean images without embedded text unless annotation was requested.",
  ...
  const BLOCK_ARCHETYPES = [
    ...
    {
      id: "image_mask_refinement",
      title: "Image Mask Refinement Block",
      purpose: "Use an image-capable agent to refine or regenerate a source-aligned mask while preserving a strict visual-output contract.",
      useWhen: "A deterministic image-analysis method misses tissue, merges instances, over-fragments texture, or needs an adjacent image-generation/refinement agent.",
      contract: ["input source_image:image", "input qc_report:json", "output prompt:markdown", "output generated_mask:image", "output integration_manifest:json", "output verifier_report:json", "no embedded image text"],
      snippet: [
        "block refine_instance_mask {",
        "  uses image_refinement_agent",
        "  input source_image: image required = \"data/source_tile.png\"",
        "  input qc_report: json required = \"artifacts/qc_report.json\"",
        "  output downstream_prompt: markdown = \"artifacts/refinement/prompt.md\"",
        "  output generated_mask: image = \"artifacts/refinement/colored_instance_mask.png\"",
        "  output integration_manifest: json = \"artifacts/refinement/integration_manifest.json\"",
        "  output verifier_report: json = \"artifacts/refinement/verifier_report.json\"",
        "  prompt \"Use only source_image as the visual reference by default. Treat previous masks and overlays as text-only QC context unless the verifier asks for extra references. Generate a colored instance mask only: one flat distinct color per visible instance, source-aligned dimensions, plain black/white/transparent background, no grayscale microscopy texture or source underlay unless this block is explicitly configured as an overlay, no text, no numbers, no arrows, no labels, no captions, no legends, no table/grid labels, and no whole-cluster single-object annotation. Record uncertainty in JSON metadata instead of drawing text.\"",
        "  validate exists \"${output.generated_mask}\"",
        "  validate json \"${output.integration_manifest}\"",
        "  validate json \"${output.verifier_report}\"",
        "  recover \"If the verifier finds embedded text, merged instances, missed tissue, or wrong dimensions, append those defects to the next prompt and regenerate within the retry limit.\"",
        "}",
      ].join("\n"),
    },
    {
