(function (root, factory) {
  ...
  const AGENT_HANDOFF_PRINCIPLES = [
    ...
    "For colored instance mask outputs, require one flat distinct color per visible instance, source-aligned dimensions, background preserved or transparent, and no text, labels, arrows, captions, legends, tables, or whole-cluster single-object annotation inside the image.",
  ...
  const BLOCK_ARCHETYPES = [
    ...
    },
    {
      ...
      snippet: [
        ...
        "  prompt \"Use only source_image as the visual reference by default. Treat previous masks and overlays as text-only QC context unless the verifier asks for extra references. Generate a colored instance mask only: one flat distinct color per visible instance, source-aligned dimensions, background preserved or transparent, no text, no numbers, no arrows, no labels, no captions, no legends, no table/grid labels, and no whole-cluster single-object annotation. Record uncertainty in JSON metadata instead of drawing text.\"",
