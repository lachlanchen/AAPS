...
We already have a general AAPS Studio and a general parser. Now I want AAPS to become a real harness around AI agents: a system where workflows are written as readable `.aaps` scripts with typed inputs/outputs, reusable functional blocks, small actions, tool routing, agent routing, if/else logic, loops, validation, recovery, artifact tracking, human review checkpoints, project management, and real execution.

AAPS should not only parse and visualize `.aaps`. It should also be able to execute `.aaps` workflows, prepare the correct runtime environment, run each functional block correctly, and repair execution failures when possible.

The complete flow should be:

`.aaps script -> project-aware parser -> intermediate representation -> execution plan -> environment/tool/agent preparation -> block execution -> artifact generation -> validation -> recovery/repair -> report`

Please inspect the current repo first. Then update the grammar, parser, project logic, web app, executable runtime, block system, examples, tests, README, and website.
...
- per-block environment requirements
- Python/shell/script preparation
...
AAPS should be useful for at least these domains:
...
The loop system must actually execute correctly, not only visualize the loop. For example, if the workflow says to segment all images in a folder, AAPS should resolve the folder, enumerate valid image files, execute the segmentation block once per image, save per-image masks/overlays/tables, and produce a combined report.

7. Tool/model/agent routing

AAPS should be able to describe candidate tools, candidate models, candidate agents, and routing logic.
...
- external segmentation CLI
- manually reviewed segmentation
...
The grammar should make routing inspectable and editable.

The runtime must also be able to resolve whether the selected tool/agent is actually available. If not available, it should provide a clear missing-tool report and suggest setup or fallback.
...
- see all environment files inside the project
- see all available tools/agents
...
- environment files used by blocks
- tools/agents used by blocks
- unsaved changes
...
- data root
- run/log paths
- available tools/models/agents from the project manifest
- script paths
- environment settings
...
Clicking an environment file should show dependencies.
...
- create or edit requirements used by blocks
...
============================================================
PART 3 — Add functional block contracts
============================================================

This is a key requirement.

Each functional block must be able to work correctly on its own, not only as a visual node.

A block should have a clear contract:

    block contract = inputs + outputs + parameters + environment + tools + agent + scripts + execution logic + validation + recovery + tests

Every functional block should be able to answer:

- What inputs do I require?
- What outputs do I produce?
- What Python interpreter or shell do I need?
- What Python packages do I need?
- What system commands/libraries do I need?
- What script files do I run?
- What inline code do I contain?
- What tool/model/agent do I use?
- How do I check that the tool/model/agent is available?
- How do I execute?
- How do I validate success?
- How do I recover from failure?
- What artifacts do I save?
- Can I be tested independently?

Please implement a block readiness/preflight system.

Before a block runs, AAPS should check:

- required inputs exist
- required input types are valid
- required project variables are resolved
- required scripts exist
- required Python interpreter exists
- required Python packages are installed
- required system commands exist
- required system libraries or external tools are available when detectable
- required model/API credentials are present if declared
- output directories are writable
- artifact paths are resolved
- selected agent/tool is available
- fallback tool/agent is available if declared

The runtime should classify each block as:
...
AAPS should show this clearly in the UI.

The block detail panel should include a “Block Readiness” section showing:
...
The block should support a local test fixture where possible:
    ...
    block segment_image:
      ...
      agent:
        optional:
          ...
          purpose: repair script if Python execution fails
      ...
      validate:
        ...
        exists: ${output.overlay_path}
        exists: ${output.object_table}
        mask_not_empty: ${output.mask_path}
    
      on_error:
        retry: 1
        fallback: simple_otsu
        repair_prompt: true
    
      test:
        sample_input: data/demo/example_image.png
        expected:
          exists: artifacts/test/example_mask.png

Use this syntax or a better one that matches the existing grammar.

============================================================
PART 4 — Make `.aaps` executable with runtime and repair logic
============================================================
...
`.aaps script -> parser -> intermediate representation -> execution plan -> environment/tool/agent preparation -> block execution -> artifact generation -> validation -> recovery/repair -> report`
...
- `agent`: call a registered agent or prepare an agent prompt
...
- agent is registered or prompt can be prepared
...
============================================================
PART 5 — Add tool registry, agent registry, and environment registry
============================================================

Please add a clear system for tools, agents, and environments.

AAPS should know what tools and agents are available before execution.

1. Tool registry

A project should be able to define tools in a file such as:

    tools/tool_registry.json

or inside `aaps.project.json`.
...
A project should be able to define agents in a file such as:
...
- codex_repair_agent
- vision_qc_agent
- segmentation_method_selector
- app_debug_agent
- writing_consistency_agent

The first implementation can prepare prompts for these agents instead of calling external APIs directly.

3. Environment registry

A project should be able to define environments in files such as:

    environments/requirements.txt
    environments/environment.yml
    environments/node_package.json
    environments/aaps_environment.json

The environment registry should support:

- project default Python interpreter
- project virtualenv path
- block-specific Python environment
...
4. Environment setup and prompt

If dependencies are missing, AAPS should not silently fail.
...
- setup prompt for the user or agent
- clear warning if package installation is not allowed

For example:

    Missing package: opencv-python
    Suggested project-local setup:
    ${project.root}/.venv/bin/pip install opencv-python

Do not install globally by default.

============================================================
PART 6 — Add block-level chat and code-authoring support
============================================================
...
- “prepare the Python requirements for this block”
- “check whether this block is executable”
- “make this block work on a folder of images”
...
5. Chat optionally writes or updates requirements.
6. Parser reparses the `.aaps`.
7. Runtime checks block readiness.
8. Runtime builds an execution plan.
9. User can dry run or run the selected block.
10. If execution fails, chat reads logs and proposes a repair.
11. User can apply repair.
12. AAPS reruns the block and updates graph status.
...
============================================================
PART 7 — Add a real segmentation folder execution example
============================================================

Please add a real, local, executable segmentation pipeline example.

This is the most important end-to-end demonstration.

The example should show that AAPS can:

1. Open a project.
2. Select a working `.aaps`.
3. Prepare/check the Python environment.
4. Find scripts and tools.
5. Loop over all images in a folder.
6. Run QC on each image.
7. Run segmentation on each image.
8. Save mask and overlay for each image.
9. Quantify each mask.
10. Save per-image CSV/JSON results.
11. Save a combined summary report.
12. Validate that outputs exist and masks are not empty.
13. Log each block execution.
14. Recover or fallback if one image fails.
15. Continue the batch if allowed.
16. Show results in the Studio.
...
Example project:
    ...
    examples/projects/organoid-analysis/
      ...
      blocks/
        ...
        segment_image.aaps
        quantify_mask.aaps
        summarize_batch.aaps
      workflows/
        executable_folder_segmentation.aaps
      scripts/
...
The workflow should conceptually do:

    workflow executable_folder_segmentation:
      input:
        image_folder: folder = data/demo_images
    
      block prepare_inputs:
        action:
          generate demo images if folder is empty
    
      for image_path in list_files(${input.image_folder}, pattern="*.png"):
        run qc_image(image_path)
        if qc_image.valid:
          run segment_image(image_path)
          run quantify_mask(mask_path)
        else:
          log skipped image
    
      run summarize_batch

The example should run locally without external APIs.

Use simple threshold segmentation first. Do not depend on Cellpose/SAM for the minimal demo. Cellpose/SAM can be optional advanced tools.

The segmentation script should actually create masks and overlays.

The quantification script should actually compute simple metrics such as:

- object count
- foreground area
- mean intensity
- bounding boxes if practical

The summary script should actually create:

- combined CSV
- JSON summary
- optional plot if dependencies allow

This demo should be used as a smoke test for the whole system.

============================================================
PART 8 — Update the web app into a real AAPS Studio
============================================================
...
- view blocks/skills/workflows/scripts/environments/tools/agents/data/artifacts/runs/reports/notes
...
- create requirements for a block
...
- block readiness viewer
- tool/agent resolution viewer
- loops shown clearly
...
- Check selected block readiness
- Run selected block
...
============================================================
PART 9 — Parser, intermediate representation, and visualization integration
============================================================
...
============================================================
PART 10 — Copy and organize representative pipeline scripts
============================================================
...
============================================================
PART 11 — Convert representative pipelines into `.aaps` examples
============================================================
...
============================================================
PART 12 — Add full example AAPS projects
============================================================
...
Example commands:
    ...
    aaps check-block workflows/main.aaps --block segment_image --project .
...
- checking block readiness
- resolving tools/agents
...
- loop over images
- run QC per image
- run segmentation per image
- save mask per image
- save overlay per image
- quantify mask per image
- save per-image result
- save combined batch report
- validate masks are not empty
- skip or fallback on one failed image
...
- functional block contracts
- block readiness/preflight
- tool registry
- agent registry
...
- how setup prompts work
- how to run workflows from CLI
- how to run the executable folder segmentation demo
...
- verify that block readiness/preflight works
- verify that tools/agents resolve correctly
- verify that executable examples run
- verify that folder segmentation demo really loops over images
- verify that masks/overlays/tables/reports are created
...
- verify that setup prompt generation works
...
- functional block contract changes
- environment/tool/agent registry changes
- runtime/execution changes
- Python/shell/block-code changes
- segmentation folder demo changes
- UI changes
...
- Make each functional block independently understandable and executable.
- Make the folder segmentation demo actually run.
