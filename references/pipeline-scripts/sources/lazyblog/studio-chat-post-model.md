...
- `GET /api/settings`: load the Studio model/reasoning profiles.
- `POST /api/settings`: save the Studio model/reasoning profiles.

Legacy `/api/draft` and `/api/publish` still exist for compatibility, but the Studio UI uses the post-project endpoints.

## Prompt Tools

- `web-action-router.txt` runs through the Studio wrapper `action` profile. The current default is `gpt-5.4` with medium reasoning.
...
Wrapper profiles now default to:

- `reply`: `gpt-5.5 / medium`
- `task`: `gpt-5.5 / high`
- `action`: `gpt-5.4 / medium`
- `response`: `gpt-5.4 / medium`
- `translation`: `gpt-5.4 / medium`
...
- `Settings`: opens a modal that edits the wrapper model/reasoning profiles persisted in `content/studio-settings.json`.
...
Concurrency follows the same philosophy:

- ordered chat state stays sequential
- independent background jobs may run in parallel

That is why the Studio can safely run multiple wrapper or translation jobs, but should not process multiple chat mutations out of order.
